...
_writeIsolate(args) {
  var data = args[0];
  Uint8List pdfBytes = data.buffer.asUint8List(
    data.offsetInBytes,
    data.lengthInBytes,
  );
...
class AppDatabase {
  ...
  static Future<Database?> openDatabaseWithTokenizer(
    ...
    Database? db;
    sqfliteFfiInit();
    final library = await Sqlite3ArabicPhoneticFuzzyTrigramFlutter()
        .loadNativeLibrary();
    if (library != null) {
      final extension = sqlite3.SqliteExtension.inLibrary(
        library,
        'sqlite3_arabic_phonetic_fuzzy_trigram_init',
      );
      print(
        "SYMBOL ${library.providesSymbol('sqlite3_arabic_phonetic_fuzzy_trigram_init')}",
      );
  ...
  Future<void> testTokenizer() async {
    ...
    final maps = await db.rawQuery(
      "SELECT rowid, highlight(docs, 0, '<b>', '</b>') as content FROM docs where content MATCH ?",
      ["الحمد"],
    );
    print("testTokenizer ${maps.length}");
    final verses = maps.map(Content.fromJson).toList();
    verses.forEach((element) => print(element));
  }

  Future<List<Content>> search(String key) async {
    Database? db = await (database);
    final maps = await db.rawQuery(
      "SELECT rowid, highlight(docs, 0, '<b>', '</b>') as content FROM docs where content MATCH ?",
      [key],
